...
function triplica(el, index, array) {
  el = el * 3;
  return el;
}
...
function filtra(el, index, array) {
  if (el > 3 && el % 2 !== 0) {
    return true;
  }
  return false;
...
function inferiore18(el, index, array) {
  if (el.age < 18) {
    return true;
  }
  return false;
...
function somma(acc, el, index, array) {
  acc = acc + el;
  return acc;
}
...
function compreso(el, index, array) {
...
function maggioreDiVenti(el, index, array) {
  if (el.age > 20) {
    return true;
  }
  return false;
}
function soloNome(acc, el, index, array) {
  return [...acc, el.name];
}
...
function soloPrice(el, index, array) {
  if (el.customer === "Alice") {
    return true;
  }
  return false;
}
function sommaPrice(acc, el, index, array) {
  acc = el.price + acc;
  return acc;
}
...
function inizialeB(el, index, array) {
...
function inizialeE(el, index, array) {
...
function conta(acc, el, index, array) {
  acc = el.length + acc;
  return acc;
}
...
function radice(el, index, array) {
  return Math.sqrt(el);
}
const radiceQuadrata = numbers4.map(radice);
console.log(radiceQuadrata);
function contaNumeri(acc, el, index, array) {
  return acc + el;
}
